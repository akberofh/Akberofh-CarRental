...
export default function AdminHome() {
  ...
  const deleteUser = async (id, name, email) => {
    if (window.confirm(`Are you sure you want to delete ${name}?`)) {
      try {
        // Delete from the local API
        const localRes = await fetch(`http://localhost:8000/api/users/${id}`, {
          method: "DELETE",
          headers: {
        ...
        if (!localRes.ok) {
          throw new Error("Network response was not ok for local API");
        }

        const localData = await localRes.json();
        if (localData) {
          toast("User deleted from local API");
          setData(prevData => prevData.filter(user => user._id !== id));
        } else {
          toast.error("An error occurred while deleting the user from local API.");
        }

        // Fetch the user from the mock API by name and email
        const mockRes = await fetch(`https://66486ec72bb946cf2fa08f2c.mockapi.io/wishlist`, {
          method: "GET",
        });

        if (!mockRes.ok) {
          throw new Error("Network response was not ok for mock API");
        }

        const users = await mockRes.json();
        console.log("Users fetched from mock API:", users);
        const userToDelete = users.find(user => user.name === name && user.email === email);
        console.log("User to delete from mock API:", userToDelete);

        if (userToDelete) {
          const deleteMockRes = await fetch(`https://66486ec72bb946cf2fa08f2c.mockapi.io/wishlist/${userToDelete.id}`, {
            method: "DELETE",
          });

          if (!deleteMockRes.ok) {
            throw new Error("Network response was not ok for mock API delete");
          }

          const deleteMockData = await deleteMockRes.json();
          if (deleteMockData) {
            toast("User deleted from mock API");
          } else {
            toast.error("An error occurred while deleting the user from mock API.");
          }
        } else {
          console.error("User not found in mock API.");
          toast.error("User not found in mock API.");
        }
      } catch (error) {
              ...
              <tr key={user._id} className="hover:bg-gray-100">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm">
                  <div className="flex items-center space-x-4">
                    ...
                    />
                    <button
                      className="px-2 py-1 text-xs rounded bg-red-500 text-white hover:bg-red-700"
                    >
                      Inactive
                    </button>
                  </div>
